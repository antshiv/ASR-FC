...
static void test_command_round_trip(void) {
    const afc_motor_command_t input = {
        .motor_index = 2,
        .armed = true,
        .flags = 3,
        .throttle_q15 = 16384,
    ...
    uint8_t frame[AFC_MOTOR_LINK_MAX_FRAME_SIZE];
    size_t frame_size = 0;
    assert(afc_motor_link_encode_command(
        42, &input, frame, sizeof(frame), &frame_size) == AFC_MOTOR_LINK_OK);
    uint32_t sequence = 0;
    afc_motor_command_t output;
    assert(afc_motor_link_decode_command(
        frame, frame_size, &sequence, &output) == AFC_MOTOR_LINK_OK);
    assert(sequence == 42);
...
static void test_corruption_fails_closed(void) {
    ...
    size_t frame_size = 0;
    assert(afc_motor_link_encode_command(
        1, &command, frame, sizeof(frame), &frame_size) == AFC_MOTOR_LINK_OK);
    frame[12] ^= 0x40u;
    uint32_t sequence = 0;
    afc_motor_command_t output;
    assert(afc_motor_link_decode_command(
        frame, frame_size, &sequence, &output) ==
        AFC_MOTOR_LINK_CRC_FAILURE);
    assert(!output.armed);
    assert(output.throttle_q15 == 0u);
}

static void test_telemetry_round_trip(void) {
    const afc_motor_telemetry_t input = {
        .motor_index = 1,
        .motor_mode = 3,
        .fault_flags = 0x22u,
    ...
    uint8_t frame[AFC_MOTOR_LINK_MAX_FRAME_SIZE];
    size_t frame_size = 0;
    assert(afc_motor_link_encode_telemetry(
        100, &input, frame, sizeof(frame), &frame_size) == AFC_MOTOR_LINK_OK);
    uint32_t sequence = 0;
    afc_motor_telemetry_t output;
    assert(afc_motor_link_decode_telemetry(
        frame, frame_size, &sequence, &output) == AFC_MOTOR_LINK_OK);
    assert(sequence == 100);
...
int main(void) {
    ...
    puts("AntshivFlightController motor-link tests passed");
    return 0;
}
